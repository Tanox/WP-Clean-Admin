...
- **2026-09-13 全量审查对上述「死代码」结论的更正**：这些 FQCN 实际由 `includes/modules/admin/classes/**` 副本成功声明（legacy 载入失败 → 无冲突 → modular 载入成功），故功能是活的，「死代码」仅指 `includes/class-wpca-menu-customizer.php` 等**根版文件本身**。同时发现更严重的反向问题：`modules/admin/classes/**` 与 `modules/utilities/classes/**` 共 35 个文件的 `namespace` 误写为根命名空间 `WPCleanAdmin`，与根版同名类 FQCN 冲突 → 运行期 `Cannot declare class` 致命错误（详见 `2026-09-13.md` 审查条目）。
...
## 运行期阻塞缺陷（2026-09-13 全量审查，静态推导，尚未实机验证）
- **插件当前可能无法运行**：`load_legacy_modules()` 与 `load_modular_modules()` 都会载入同名类，而 `modules/admin/**`、`modules/utilities/**` 的 namespace 误写为 `WPCleanAdmin`（应与目录一致），导致 `Cannot declare class WPCleanAdmin\Dashboard` 之类致命错误（E_COMPILE_ERROR，`safe_init` 的 try/catch 无效）。修复方向：把这 35 个文件的 `namespace` 改为对应子命名空间（`WPCleanAdmin\Modules\Admin\Classes` / `...\Utilities\Classes`），或让 modular 只载入 legacy 未声明的类。
- **第二处 fatal**：`includes/modules/utilities/classes/class-wpca-helpers.php:18` require 了不存在的 `wpca-wordpress-stubs.php`（该目录无此文件）。
- **根本流程缺口**：CI 从不 boot 插件（php -l + 3 个不加载主文件的测试），此类致命错误无法被 CI 发现；建议给 CI 加「冒烟加载检查」。
- **安全**：2FA 存在认证绕过（`class-wpca-login-two-factor-tasks.php:276-294`）；menu-customizer 的 4 个 `wp_ajax_` 处理器缺 nonce/权限校验。
